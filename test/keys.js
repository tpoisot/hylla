describe('Keys', function() {
  ...
  describe('names', function() {
    ...
    it('should return the institution name if the first author is an institution', function() {
      ...
      chai.assert.equal(shelf.keys.nameOfFirstAuthor(lib.entry("inte09ccp").content), "IntergovernmentalPanelonClimateChange");
    });

    it('should remove diacritics', function() {
      var lib = new shelf.Library(path.resolve("./test/data"));
      chai.assert.equal(shelf.keys.nameOfFirstAuthor(lib.entry("lali10dht").content), "Laliberte");
    });
  ...
  describe('date', function() {
    it('should return the issued year if present', function () {
      var lib = new shelf.Library(path.resolve("./test/data"));
      chai.assert.equal(shelf.keys.entireYear(lib.entry("lali10dht").content), "2010");
    });
    it('should work with the shortened year too', function () {
      var lib = new shelf.Library(path.resolve("./test/data"));
      chai.assert.equal(shelf.keys.lastDigitsOfYear(lib.entry("lali10dht").content), "10");
    });
  ...
  describe('title', function() {
    ...
    it('should return the title first letters', function () {
      var lib = new shelf.Library(path.resolve("./test/data"));
      chai.assert.equal(shelf.keys.firstLettersOfTitle(lib.entry("lali10dht").content), "dht");
    });
