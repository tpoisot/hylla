...
function citeprocizeRef(data) {
  ...
  var alwaysRemove = ['license', 'source', 'prefix', 'member', 'score',
    ...
    'keywords', 'update-policy', 'abstract', 'link', 'ISSN',
    'published-print', 'published-online', 'deposited', 'indexed', 'created',
    'article-number', 'archive'
  ];
  alwaysRemove.map(function (e) {
    delete data[e];
    return e;
  });
  ...
  // Fix problematric fields
  if (Array.isArray(data.ISSN)) data.ISSN = String(data.ISSN[0]);
  if (Array.isArray(data.ISBN)) data.ISBN = String(data.ISBN[0]);
  // Return
  return data;
}
...
Library.prototype.json = function (ids) {
  ...
  return JSON.stringify(entries.map(function (e) {
    return citeprocizeRef(e.content);
  }), null, 2);
};
